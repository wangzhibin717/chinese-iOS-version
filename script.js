document.addEventListener('DOMContentLoaded', () => {
    ...
    function initGroupSelect() {
        groupSelect.innerHTML = '<option value="all">全部生字</option>';
        const total = originalFullData.length;
        if (total === 0) return;
        ...
        for (let i = 0; i < groupCount; i++) {
            ...
            const endIdx = Math.min((i + 1) * groupSize, total);
            const option = document.createElement('option');
            option.value = `group-${i}`;
            option.textContent = `第 ${i + 1} 组 (${startIdx + 1}-${endIdx})`;
    ...
    function handleGroupChange() {
        ...
        if (val === 'all') {
            newData = [...originalFullData];
        } else if (val.startsWith('group-')) {
            const groupIndex = parseInt(val.split('-')[1]);
            const groupCount = 3;
            const groupSize = Math.ceil(originalFullData.length / groupCount);
            const start = groupIndex * groupSize;
            const end = Math.min((groupIndex + 1) * groupSize, originalFullData.length);
            newData = originalFullData.slice(start, end);
        } else {
            newData = [...originalFullData];
        }

        // Reset practice mode if active, as user explicitly changed scope
        if (practiceEnabled) {
            exitDemoPractice(); 
        }

        setBaseData(newData);
        applyQuizData(newData);
    }

    groupSelect.addEventListener('change', handleGroupChange);

    // Initial Data Setup
    originalFullData = Array.isArray(quizData) ? [...quizData] : [];
    initGroupSelect();
    
    // Ensure baseQuizData matches initial state (all)
    setBaseData(originalFullData);
    ...
    confirmImportBtn.addEventListener('click', () => {
        ...
        try {
            ...
            // Update original data and re-init groups
             originalFullData = [...newData];
             initGroupSelect();
             groupSelect.value = 'all';
             
             setBaseData(originalFullData);
            applyQuizData(originalFullData);
            updateDemoUI();
